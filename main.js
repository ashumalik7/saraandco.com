// ==========================================
// 1. DYNAMIC INVENTORY HUB
// ==========================================
const jewelryProducts = [
    { id: "ring-solitaire-01", name: "Timeless Solitaire Diamond Band", category: "Rings", price: 499, desc: "Minimal diamond band.", baseFolder: "images/rings/ring-solitaire-01", hasDiscount: true, discountPrice: 399 },
    { id: "ring-vintage-gold-02", name: "The Vintage style Gold Band", category: "Rings", price: 599, desc: "Antique gold band.", baseFolder: "images/rings/ring-vintage-02", hasDiscount: true, discountPrice: 449 },
    { id: "ring-minimal-band-03", name: "Minimal Sleek Comfort Band", category: "Rings", price: 299, desc: "Thin micro pave band.", baseFolder: "images/rings/ring-minimal-band-03", hasDiscount: true, discountPrice: 199 },
    { id: "neck-royal-heritage-01", name: "Sara Royal Heritage Choker", category: "Necklaces", price: 999, desc: "Festive heavy filigree.", baseFolder: "images/necklaces/neck-royal-heritage-01", hasDiscount: true, discountPrice: 799 },
    { id: "ear-jhumka-traditional-01", name: "Imperial Filigree Drop Jhumkas", category: "Earrings", price: 399, desc: "Traditional hanging columns.", baseFolder: "images/earrings/ear-jhumka-traditional-01", hasDiscount: true, discountPrice: 299 },
    { id: "brac-cuff-luxury-01", name: "Eclipse Matte Statement Cuff", category: "Bracelets", price: 499, desc: "Solid matte satin gold.", baseFolder: "images/bracelets/brac-cuff-luxury-01", hasDiscount: true, discountPrice: 349 },
    { id: "brac-chain-minimal-02", name: "Minimalist Chain Link Bracelet", category: "Bracelets", price: 199, desc: "Delicate chain link.", baseFolder: "images/bracelets/brac-chain-minimal-02", hasDiscount: true, discountPrice: 149 }
];

window.jewelryProducts = jewelryProducts;
window.cartProductsList = JSON.parse(localStorage.getItem('myCart')) || [];

// ==========================================
// 2. CART RENDERER (Updated with Calculations)
// ==========================================
// Cart Render with + and - Buttons
window.calculateAndRenderCart = function() {
    localStorage.setItem('myCart', JSON.stringify(window.cartProductsList));
    ...
    // Grouping logic
    const grouped = window.cartProductsList.reduce((acc, curr) => {
    ...
    grouped.forEach((product) => {
        ...
        container.insertAdjacentHTML('beforeend', `
            <div class="flex items-center justify-between border-b pb-4 text-xs">
                <div class="flex items-center space-x-3">
                    <img src="${product.imgSrc}" class="w-12 h-14 object-contain">
                    <div>
                        <h4 class="font-serif">${product.title}</h4>
                        <div class="flex items-center space-x-2 mt-1">
                            <button onclick="updateQty('${product.title}', -1)" class="px-2 py-0.5 bg-stone-100 hover:bg-stone-200">-</button>
                            <span>${product.qty}</span>
                            <button onclick="updateQty('${product.title}', 1)" class="px-2 py-0.5 bg-stone-100 hover:bg-stone-200">+</button>
                        </div>
                    </div>
                </div>
                <span class="font-bold">₹${(product.price * product.qty).toLocaleString('en-IN')}.00</span>
            </div>
        `);
    });

    if (grouped.length === 0) container.innerHTML = '<p class="text-xs text-stone-400 text-center pt-8">Cart is empty.</p>';
    
    // Totals
    if(document.getElementById('cartSubtotalAmount')) document.getElementById('cartSubtotalAmount').innerText = "₹" + subtotal.toLocaleString('en-IN') + ".00";
    if(document.getElementById('cartGrandTotalAmount')) document.getElementById('cartGrandTotalAmount').innerText = "₹" + subtotal.toLocaleString('en-IN') + ".00";
};

// 2. Quantity Update Logic
window.updateQty = function(title, change) {
    const index = window.cartProductsList.findIndex(item => item.title === title);
    if (index > -1) {
        if (change === 1) {
            // Add item
            const item = window.cartProductsList[index];
            window.cartProductsList.push({ ...item, qty: 1 });
        } else {
            // Remove one instance
            window.cartProductsList.splice(index, 1);
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.renderSeparatedGrids = function() {
        ...
        jewelryProducts.forEach(product => {
            ...
            const cardHTML = `
                <div class="group cursor-pointer flex flex-col space-y-4" onclick="window.location.href='product-page.html?id=${product.id}'">
    ...
    // Data save karo taaki refresh hone par na ude
    localStorage.setItem('myCart', JSON.stringify(cartProductsList));
    ...
    // Calculations
    let deliveryCharge = (subtotalSum > 0 && subtotalSum < 500) ? 40 : 0;
    let grandTotal = subtotalSum + deliveryCharge;

    const subTotalEl = document.getElementById('cartSubtotalAmount');
    if(subTotalEl) subTotalEl.innerText = "₹" + subtotalSum.toLocaleString('en-IN') + ".00";
    
    const savingsEl = document.getElementById('cartSavingsAmount');
    if(savingsEl) savingsEl.innerText = "-₹" + totalSavings.toLocaleString('en-IN') + ".00";
    
    const delEl = document.getElementById('cartDeliveryAmount');
    if(delEl) delEl.innerText = deliveryCharge === 0 ? (subtotalSum >= 500 ? "FREE" : "₹0.00") : "₹" + deliveryCharge + ".00";
    
    const grandEl = document.getElementById('cartGrandTotalAmount');
    if(grandEl) grandEl.innerText = "₹" + grandTotal.toLocaleString('en-IN') + ".00";

    const badge = document.getElementById('floatingCartBadge');
    if (badge) {
        badge.innerText = globalCount;
        badge.classList.toggle('opacity-0', globalCount === 0);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    searchInput.addEventListener('input', (e) => {
        ...
        matchedProducts.forEach(product => {
            const rowHTML = `
                <div onclick="window.location.href='product-page.html?id=${product.id}'" class="flex items-center justify-between px-4 py-2 hover:bg-stone-50 cursor-pointer transition-colors border-b border-stone-100/40 last:border-0 group/row">
    ...
    // Ab search select karne par seedha naye page par jayega
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    searchInput.addEventListener('input', (e) => {
        ...
        matched.forEach(product => {
            const row = `
                <div onclick="window.location.href='product-page.html?id=${product.id}'" class="flex items-center justify-between px-5 py-3 hover:bg-stone-50 cursor-pointer transition-colors border-b border-stone-100 last:border-0 group">
    ...
    // Ab search select karne par seedha naye page par redirect hoga
...
// ==========================================
// 5. ADD TO CART - FINALIZED LOGIC
// ==========================================

// Ye function Modal ke Acquire button se connect hoga
window.addToCartPage = function() {
    if(!selectedSize) { 
    ...
    const p = jewelryProducts.find(item => item.id === productId);
    
    if(p) {
        // 2. Data push karo
        const item = {
            title: p.name,
            price: p.hasDiscount ? p.discountPrice : p.price,
            imgSrc: p.baseFolder + "-front.jpg",
            qty: 1
        };
        ...
        console.log("Cart List abhi:", cartProductsList); // F12 daba ke console mein check kar
        
        // 3. Render call karo
        if (typeof calculateAndRenderCart === 'function') {
            calculateAndRenderCart();
        }
    } else {
        console.error("Product nahi mila!");
    }
